...
export class AuthService {
	...
	constructor(private http: Http, private alertService : AlertService,private router: Router){		
		var tokenStorage = JSON.parse(localStorage.getItem('pokemonTokenStorage'));
		this.token = tokenStorage && tokenStorage.token;
		this.loginUrl = Constants.END_POINT.login + 'auth';
	}
	...
	login(username: string, password: string, grant: string): Observable<any>{
		...
		return this.http.post(this.loginUrl,body, {headers: this.headers})
			.map((response: Response) => {				
				...
				if(token){
					this.token = token;					
					localStorage.setItem('pokemonTokenStorage', JSON.stringify(retorno));		
	...
	logout(): void{
		this.token = null;
		localStorage.removeItem('pokemonTokenStorage');
	...
	getToken(){		
		let tokenStorage = JSON.parse(localStorage.getItem('pokemonTokenStorage'));			
